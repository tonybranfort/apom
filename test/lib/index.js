...
function getPropDefns(props, options) {
  // returns an array of propDefns - typically one per each property - 
  //    replacing default values for each propDefn property that isn't included
...
function getPropRefs (pObj, tObj, propName, callback) {
  ...
  getPropRef(pObj, propNameKeys, function(err, pObjPropRef) {
    getPropRef(tObj, propNameKeys, function(err, tObjPropRef) {
      var retPropRefs = {pObjProp: pObjPropRef, tObjProp: tObjPropRef};
      return callback(err, retPropRefs);   
    });
  });

  // async.parallel( [
  //   function(cb) {
  //     getPropRef(pObj, propNameKeys, cb); 
  //   }, 
  //   function(cb) {
  //     getPropRef(tObj, propNameKeys, cb); 
  //   }
  //   ], 
  //   function(err, res) {
  //     if(err) {
  //       return callback(err); 
  //     }
  //     return callback(null, {pObjProp: res[0], tObjProp: res[1]}); 
  //   }
  //   ); 
  ...
  function getPropRef (obj, propNameKeys, callback) {
    ...
    async.whilst(
      ...
      function(err) {  //called when test finally fails
        ...
        return callback(null, {value: propRef, exists: propExists}); 
      }
      );
